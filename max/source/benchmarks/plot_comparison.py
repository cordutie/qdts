...
line_s, = ax.plot(Ns, solver_mean, "o-", color=BLUE)
band_s  = ax.fill_between(Ns, lo_s, hi_s, alpha=0.20, color=BLUE, linewidth=0)

lo_n = [m - s for m, s in zip(nn_mean, nn_std)]
hi_n = [m + s for m, s in zip(nn_mean, nn_std)]
line_n, = ax.plot(Ns, nn_mean, "s-", color=ORANGE)
band_n  = ax.fill_between(Ns, lo_n, hi_n, alpha=0.20, color=ORANGE, linewidth=0)
...
h_s = (Line2D([0], [0], color=BLUE,   marker="o", linewidth=1.5, markersize=5),
       Patch(facecolor=BLUE,   alpha=0.30, linewidth=0))
h_n = (Line2D([0], [0], color=ORANGE, marker="s", linewidth=1.5, markersize=5),
